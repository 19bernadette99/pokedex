...
function fetchPokemonList(append = false) {
  ...
  fetch(url)
    ...
    .then((data) =>
      Promise.all(data.results.map((pokemon) => fetchPokemonData(pokemon.url)))
    )
    .then((newPokemonData) =>
      displayPokemon(newPokemonData.filter(Boolean), append)
    );
}
...
function fetchMorePokemon(pokemonList) {
  ...
  Promise.all(pokemonDetailsPromises).then((pokemonData) => {
    pokemonData.forEach((data) => pokemonArray.push(data));
    displayPokemon(pokemonArray, true);
  });
}

function createPokemonCardElement(
  pokemon,
  firstType,
  bgColor,
  imageUrl,
  ...
  card.innerHTML = createPokemonCard(
    pokemon,
    bgColor,
    imageUrl,
    name,
  ...
  if (name.length > 12)
    card.querySelector(".pokemon-name").style.fontSize = "12px";
  card.addEventListener("click", () =>
    openOverlay(pokemon, pokemonArray, pokemonArray.indexOf(pokemon))
  );
  return card;
}
...
function displayPokemon(pokemonArray, append = false) {
  ...
  pokemonArray.forEach((pokemon) => {
    ...
    const card = createPokemonCardElement(
      pokemon,
      firstType,
      bgColor,
      imageUrl,
...
function fetchPokemonDetails(filteredPokemon) {
  const promises = filteredPokemon.map((pokemon) =>
...
function showNoResults() {
  ...
  container.innerHTML = `
      <p class="no-results-message">
        ❌ No Pokémon found. Try a different name!
      </p>`;
}
